...
describe("Ceate/edit curvals", () => {
    ...
    it("Create a record in curval table and select it", () => {
        ...
        cy.contains('fieldset', 'CURVAL FIELD TEST').within(() => {
            ...
            // Re-query the list items after search
            cy.get('ul.select__menu li.answer:visible')
            .contains('RECORD 1') // Fixed typo
            .click({ force: true });

            // Re-query the selected item container
    ...
    it("Change curval type to multi val no selector and save a record", () => {
        ...
        .parent() // move to label container
        .siblings('.input__field') // get the input container
        .find('input') // find the input
        .should(($input) => {
            const val = $input.val()
            // Assert the original value (ignoring extra spaces)
            expect(val).to.match(/RECORD 1\s+HELLO “ ’/)
        })
        .clear() // clear the field
        .type('Other Value'); // type the new value
    ...
    it("Save multiple curvals  to a record", () => {
        ...
        // Dropdown Field (choose "Red" as an example)
