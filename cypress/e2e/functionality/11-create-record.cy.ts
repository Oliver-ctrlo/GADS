...
describe("Record create/edit tests", () => {
    ...
    it("It should create a record", () => {
        ...
        cy.log("select the first available option in all dropdowns field");
        ...
        cy.log("Add a value to all integer fields");
    ...
    it("It should test for manadatory errors followed by a successful create record", () => {
        cy.contains('a', 'Add a record').click();
        //How many inputs are required on this form?
    ...
    it("Should show datatype errors and then successfully create a record", () => {
        ...
        cy.log("Add invalid values to all date and date range inputs");
        ...
        cy.log("Select the first option in all dropdown fields");
        cy.get('.linkspace-field[data-column-type="enum"]')
        .each(($dropdown) => {
            cy.wrap($dropdown)
        .find('div.select-widget-dropdown > div.form-control > ul.current')
  ...
  cy.get('input[type="number"]')
    ...
    cy.log("Correct date values");
    cy.get('.modal-content')
    .contains('button.btn-cancel', 'Cancel')
    .should('be.visible')
    .click();
