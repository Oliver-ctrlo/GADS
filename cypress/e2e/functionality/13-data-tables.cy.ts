...
describe('Test data table features', () => {
   ...
   // cy.addUserToDefaultGroup('test@example.com');
    ...
    function columnSearch() {
        cy.get('button#search-toggle-1').click();
        cy.get('div[data-ddl="ddl_1"] input[type="text"]').type('RECORD 1');
        ...
        cy.get('#DataTables_Table_0_info').should('have.text', 'Showing 1 to 1 of 1 entry');
        cy.get('button#search-toggle-1').click();
        cy.get('.dropdown-menu.p-2.show').find('button.data-table__clear').contains('Clear filter').click({ force: true });
        cy.get('#DataTables_Table_0_info').should('not.have.text', 'Showing 1 to 1 of 1 entry');
        cy.get('table#DataTables_Table_0 tbody tr').should('have.length.greaterThan', 1);
        
        //col search with type ahead
        cy.get('button#search-toggle-2').click();
        cy.get('div[data-ddl="ddl_2"] input[type="text"]').type('Bl');
